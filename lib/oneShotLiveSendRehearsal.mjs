...
export function buildOneShotLiveSendRehearsal({
  ...
  const destinationUrl = preflight?.estimateReport?.params?.destinationUrl ?? preflight?.destinationUrl ?? "";
  const profile = preflight?.chainProfile ? getHaloChainProfile(preflight.chainProfile) : getHaloChainProfile();
  let sendParams = null;
  ...
  if (liveSendEnabled) {
    const webhookIssue = validateLiveOneShotWebhookUrl(destinationUrl);
    if (webhookIssue) {
      issues.push(webhookIssue);
    }
  }

  const mainnetReadiness = validateMainnetLiveSendReadiness({
    profile,
    liveSendEnabled,
    mainnetArmed: process.env.HALO_MAINNET_DEMO_ARMED === "1",
    webhookUrlReady: isLiveOneShotWebhookUrlReady(destinationUrl),
    estimateSucceeded: classification.classification === ESTIMATE_RESULT_CLASSIFICATION.SUCCEEDED,
    plannedGrantAmountAtoms: preflight?.grantAmountAtoms,
    plannedRelayerFeeAtoms: plannedFeeAmount,
  });

  if (profile.mainnet && liveSendEnabled && !mainnetReadiness.ready) {
    issues.push(...mainnetReadiness.issues);
  }
  ...
  return {
    ...
    status,
    chainProfile: preflight?.chainProfile ?? profile.id,
    chainLabel: preflight?.chainLabel ?? profile.label,
    chainId: preflight?.chainId ?? null,
    endpoint: preflight?.endpoint ?? null,
    destinationUrl: destinationUrl || null,
    webhookUrlReadyForLiveSend: isLiveOneShotWebhookUrlReady(destinationUrl),
    liveSendEnabled,
    estimateSucceeded: classification.classification === ESTIMATE_RESULT_CLASSIFICATION.SUCCEEDED,
...
export function formatOneShotLiveSendRehearsalLogs(rehearsal) {
  const lines = [
    ...
    `[CHAIN] profile=${rehearsal.chainProfile ?? "unknown"}, label=${rehearsal.chainLabel ?? "unknown"}, chainId=${rehearsal.chainId ?? "unknown"}.`,
    `[1Shot] webhook callback=${rehearsal.destinationUrl ?? "missing"}.`,
    `[1Shot] webhook live-ready=${rehearsal.webhookUrlReadyForLiveSend}.`,
    ...
    `[MAINNET] armed=${rehearsal.mainnetReadiness?.armed ?? false}, ready=${rehearsal.mainnetReadiness?.ready ?? false}.`,
